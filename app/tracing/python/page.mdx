...
Here's a quick overview of the available helpers in the Handit.ai Python SDK:

| Helper | Purpose | Sync | Async |
|--------|---------|------|-------|
| `track_model` | Wraps any model that implements `__call__` | ✔️ | ✔️ |
| `track_tool` | Wraps a LangChain Tool or callable | ✔️ | ✔️ |
| `start_agent_tracing` | Decorator for full agent runs | ✔️ | ✔️ |
| `trace_agent_node` | Decorator for a single function / node | ✔️ | ✔️ |
| `trace_agent_node_func` | Trace arbitrary call site | – | ✔️ |
| `trace_agent_node_func_sync` | Sync counterpart of above | ✔️ | – |

For the full signature of every helper, visit [API › Python SDK](/sdk/python) in the sidebar.  
