...
Follow next steps for tracing:

<Steps>

### Create a service file to initialize the tracker
### Set up the main agent function with tracing
### Add manual tracking for LLM calls or tool usage
...
Then implement your agent code:

<Tabs items={["Python", "JavaScript"]} defaultIndex="0">
<Tabs.Tab>
```python filename="agent_tracing.py"
"""
Agent tracing implementation with Handit.ai.
This example shows how to:
1. Track the main agent function
2. Manually track authentication
3. Manually track intent classification
"""
...
async def process_user_request(user_message: str, user_id: str):
    """
    Process a user request with comprehensive tracing.
    ...
    await tracker._send_tracked_data(
        ...
        response_body={
            "authenticated": True,
            "user_tier": "premium",
            "processing_time": time.time() - auth_start
        }
    )
 ...
 * 1. Track the main agent function
 * 2. Manually track authentication
 * 3. Manually track response generation
 */
 ...
 */
const processUserRequest = startAgentTracing(async (userMessage, userId) => {
    ...
    // Manual tracking of response generation
    const responseStart = Date.now();
    const response = await generateResponse(intent, userMessage);
    await captureAgentNode({
        agentNodeSlug: 'response-generation',
        requestBody: {
            intent,
            userMessage,
            userId,
        ...
        responseBody: {
            response,
            processingTime: Date.now() - responseStart,
