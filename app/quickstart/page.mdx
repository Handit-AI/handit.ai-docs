...
Now, let's implement the main agent function with tracing. You'll need to set up three key components:
1. Main orchestration function with `start_agent_tracing()`
2. LLM calls tracking with `_send_tracked_data()`
3. Tool usage tracking with `_send_tracked_data()`
