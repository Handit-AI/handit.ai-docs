...
Now, set up your main agent function with tracing. For manual tracking of LLM calls and tool usage, use `tracker._send_tracked_data(model_id, request_body, response_body)`. The `model_id` should match the slug from your `handit.config.json` [See Step 3](#step-3-create-your-agent-structure).
...
async def handle_customer_request(user_message: str, user_id: str) -> Dict[str, Any]:
    ...
    await tracker._send_tracked_data(
        model_id="customer-service-interaction",  # Must match slug in handit.config.json
        request_body={
            "user_id": user_id,
        ...
        response_body={
            ...
            "kb_results_used": len(kb_results["results"])
        }
    )
...
const handleCustomerRequest = startAgentTracing(async (userMessage, userId) => {
    ...
    // Track the complete interaction
    await captureAgentNode({
        agentNodeSlug: 'customer-service-interaction',  // Must match slug in handit.config.json
        requestBody: {
            userId,
            message: userMessage,
            timestamp: startTime
        ...
        responseBody: {
            ...
            kbResultsUsed: kbResults.results.length
        }
    });
