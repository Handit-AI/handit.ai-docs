...
Now, set up your main agent function, LLM calls and tool usage.
...
```python {14, 44-48,72-76, 81} filename="customer_service_agent.py"
"""
...
- `captureAgentNode()`
  - Type: Async function
  - Parameters:
    - `agentNodeSlug`: string (required) - Unique identifier for the Tool or LLM you want to track
    - `requestBody`: object (required) - Input data for operation
    - `responseBody`: object (required) - Output data from operation
  - Purpose: Tracks individual component executions (LLMs/Tools)

```javascript {13,38-42, 64-68, 89} filename="customer_service_agent.js"
/**
 * Customer Service Agent with comprehensive tracing.
 * 
 * This example demonstrates the correct pattern for implementing tracing in a customer service agent:
...
async function searchKnowledgeBase(query) {
    ...
    await captureAgentNode({
        agentNodeSlug: 'knowledge-base-search',  // Create your own unique identifier for your component. Must be unique across all nodes
        requestBody: { query },
...
async function generateResponse(context) {
    ...
    // Track the LLM response generation
    await captureAgentNode({
        agentNodeSlug: 'response-generator',  // Unique identifier
        requestBody: context,
